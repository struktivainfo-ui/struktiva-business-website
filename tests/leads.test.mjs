...
test('accepts a valid Digital-Check lead and sends internal plus confirmation mail', async () => {
  ...
  assert.match(deliveries[0].html, /129 € einmalig inkl\. 19 % MwSt\./)
  assert.match(deliveries[0].html, /Regulärer Preis/)
  assert.doesNotMatch(deliveries[0].html, /Einführungs(?:preis|platz)/)
  ...
  assert.doesNotMatch(deliveries[1].html, /Einführungs(?:preis|platz)|Danach 129/)
