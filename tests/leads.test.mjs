...
test('accepts a valid Digital-Check lead and sends internal plus confirmation mail', async () => {
  ...
  assert.match(deliveries[0].html, /Kostenlos/)
  assert.match(deliveries[0].html, /kein kostenpflichtiger Auftrag/)
  assert.doesNotMatch(deliveries[0].html, /(?:79|129)\s*€|Einführungs(?:preis|platz)|MwSt/)
  assert.match(deliveries[1].html, /eine unverbindliche Anfrage/)
  assert.match(deliveries[1].html, /Digital-Check ist kostenlos/)
  assert.match(deliveries[1].html, /kein kostenpflichtiger Auftrag/)
  assert.doesNotMatch(`${deliveries[0].html}${deliveries[1].html}`, /(?:79|129)\s*€|Einführungs(?:preis|platz)|MwSt|zzgl\.|zuzüglich|Kleinunternehmer|§\s*19/i)
})
